...
public class LastValueWindowFunctionTest {

    private static String path = System.getProperty("user.dir") + "/generatedSQLs/WindowFunctions/LAST_VALUE/";
    ...
    public static String getNoWindowClauseTestCases(String tableName) {
        ...
        for (NullsPolicy nullsPolicy : NullsPolicy.values())
            for (OrderBy ob : OrderBy.values()) {
                ...
                result.append("SELECT column_1, LAST_VALUE(column_1");
        ...
        for (NullsPolicy nullsPolicy : NullsPolicy.values())
            for (PartitionByType partition : PartitionByType.values())
                for (OrderBy ob : OrderBy.values()) {
                    ...
                    result.append("SELECT column_1, column_2, LAST_VALUE(column_1");
                    if (!nullsPolicy.equals(NullsPolicy.Absent))
                        result.append(" ").append(nullsPolicy.get());
    ...
    public static String getRowsFrameClauses(String tableName) {
        ...
        String pattern = "SELECT column_1, LAST_VALUE(column_1) OVER(%s) FROM " + tableName + ";\n";
    ...
    public static String getWindowClause(String tableName) {
        return "-- ************** WINDOW CLAUSE TESTS **************\n" +
                ...
                "SELECT column_1, column_2, LAST_VALUE(column_1) OVER  w FROM " + tableName + " WINDOW w AS (PARTITION BY column_2 ORDER BY column_1);\n\n";
    ...
    public static void createTests() {
        for (DataType type : typesToTest) {
            ...
            String tableName = "LAST_VALUE_" + type.toString() + "_table";
