...
public class SumWindowFunctionTest {

    private static String path = System.getProperty("user.dir") + "/generatedSQLs/WindowFunctions/SUM/";
    ...
    private static String addTestCase(String tableName, MaxMinOptions mo, OrderBy ob, String message, PartitionByType partition) {
        StringBuilder result = new StringBuilder();
        result.append(message);
        result.append("SELECT column_1, SUM(");
        if (!mo.equals(MaxMinOptions.Absent))
            result.append(mo.get()).append(" ");
        result.append("column_1) OVER (");
        if (partition != null)
    ...
    public static String getNoWindowClauseTestCases(String tableName) {
        ...
        for (MaxMinOptions mo : MaxMinOptions.values())
            for (OrderBy ob : OrderBy.values()) {
                if (mo.equals(MaxMinOptions.Distinct) && !ob.equals(OrderBy.Default))
                    negativeCases.append(addTestCase(tableName, mo, ob, "-- (NEGATIVE) CASE: Max/Min additional option [" + mo.toString() + "] Ordering type [" + ob.toString() + "]\n", null));
                else
                    result.append(addTestCase(tableName, mo, ob, "-- (POSITIVE) CASE: Max/Min additional option [" + mo.toString() + "] Ordering type [" + ob.toString() + "]\n", null));
            }
    ...
    public static String getPartitionClauses(String tableName) {
        ...
        for (MaxMinOptions mo : MaxMinOptions.values())
            for (PartitionByType partition : PartitionByType.values())
                for (OrderBy ob : OrderBy.values()) {
                    if (mo.equals(MaxMinOptions.Distinct) && !ob.equals(OrderBy.Default))
                        negativeCases.append(addTestCase(tableName, mo, ob, "-- (NEGATIVE) CASE: PARTITION TYPE [" + partition.get() + "] Ordering type [" + ob.toString() + "]\n", partition));
                    else
                        result.append(addTestCase(tableName, mo, ob, "-- (POSITIVE) CASE: PARTITION TYPE [" + partition.get() + "] Ordering type [" + ob.toString() + "]\n", partition));
                }
        result.append("\n").append(negativeCases);
        return result.toString();
    }
    ...
    public static String getRowsFrameClauses(String tableName) {
        ...
        String pattern = "SELECT column_1, SUM(column_1) OVER(%s) FROM " + tableName + ";\n";
    ...
    public static String getWindowClause(String tableName) {
        return "-- ************** WINDOW CLAUSE TESTS **************\n" +
                ...
                "SELECT column_1, column_2, SUM(column_1) OVER  w FROM " + tableName + " WINDOW w AS (ORDER BY column_2);\n\n";
    ...
    public static void createTests() {
        for (DataType type : typesToTest) {
            ...
            String tableName = "SUM_" + type.toString() + "_table";
            ...
            changes.put("[INSERT_DATA]", getSmallInserts(tableName, new DataType[]{type, type}, rowCount));
