...
public class RowNumberWindowFunctionTest {

    private static String path = System.getProperty("user.dir") + "/generatedSQLs/WindowFunctions/ROW_NUMBER/";
    ...
    private static String addTestCase(String tableName, OrderBy ob, String message, PartitionByType partition) {
        ...
        result.append("SELECT column_1, ROW_NUMBER() OVER (");
        if (partition != null)
    ...
    public static String getRowsFrameClauses(String tableName) {
        ...
        String pattern = "SELECT column_1, ROW_NUMBER(column_1) OVER(%s) FROM " + tableName + ";\n";
    ...
    public static String getWindowClause(String tableName) {
        return "-- ************** WINDOW CLAUSE TESTS **************\n" +
                ...
                "SELECT column_1, column_2, ROW_NUMBER(column_1) OVER  w FROM " + tableName + " WINDOW w AS (ORDER BY column_2);\n\n";
    ...
    public static void createTests() {
        for (DataType type : typesToTest) {
            ...
            String tableName = "DENSE_RANK_" + type.toString() + "_table";
